...
import com.cdk.ats.plugs.utils.ReceiveStrategyFactory;
...
/**
 * 
 * @Title: TypeProcess.java
 * @Package com.cdk.ats.udp.process
 * @Description: 命令处理类
 * @author 陈定凯
 * @date 2015年5月13日 下午5:20:16
 * @version V1.0
 */
public class TypeProcess {
	...
	private void receiverType() {
		...
		if (ft == 0x40) {
			...
			if (ResetContext.isReset(dp)) {
				...
				if (ActionContext.hasRestKey(CommandTools
						.formateSequnceToInt(dp.getData()))) {
					...
					if (st == 0x24) {
						...
						// 如果处理链接状态，则不处理此数据包
						log.error("异常数据包"
								+ CommandTools.formateSequnceToInt(dp.getData())
								+ ",设备" + CommandTools.getOne(dp.getData()));
			...
			if (ft == 0x01 || ft == 0x02 || ft == 0x10 || ft == 0x20
					|| ft == 0x30) {
				autoCmd();

			} else if (ft == 0x50 || ft == 0x60) {
				initCmd();
			}
		}
	}
	 ...
	 * 
	 * 
	 * @Description: 处理普通输入输出命令
	 * @author 陈定凯
	 * @date 2015年5月13日 下午4:04:13
	 */
	 ...
	 * 
	 * 
	 * @Description: 其它命令则判断是不是装配的插件命令。
	 * @author 陈定凯
	 * @date 2015年5月13日 下午4:42:31
	 */
	private void plugsCmd() {
		IPlugsReceiveCmdBiz receive = ReceiveStrategyFactory.instance()
				.getReceiver(getCmd());
		if (receive != null) {
			receive.receive(dp);
		}
	 ...
	 * @Description: 设备状态
	 * @author 陈定凯
	 * @date 2015年5月13日 下午4:23:48
	 */
	private void autoCmd0x02() {
	 ...
	 * @Description: 系统设置
	 * @author 陈定凯
	 * @date 2015年5月13日 下午4:24:04
	 */
	private void autoCmd0x10() {
		Operating_0x10 ox10 = new Operating_0x10();
		ox10.setDp(dp);
	 ...
	 * @Description: 用户操作
	 * @author 陈定凯
	 * @date 2015年5月13日 下午4:24:15
	 */
	private void autoCmd0x20() {
	 ...
	 * @Description: 数据传输
	 * @author 陈定凯
	 * @date 2015年5月13日 下午4:23:21
	 */
	private void autoCmd0x30() {
	 ...
	 * @Description: 获取命令串
	 * @author 陈定凯
	 * @date 2015年5月13日 下午8:24:30
	 * @return
	 */
	public String getCmd() {
		return "0x"
				+ StringUtils.leftPad(Integer.toHexString(ft).toLowerCase(), 2,
						"0")
				+ "-0x"
				+ StringUtils.leftPad(Integer.toHexString(st).toLowerCase(), 2,
						"0");
	}
}
