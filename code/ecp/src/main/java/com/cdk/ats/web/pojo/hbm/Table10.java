...
public class Table10 implements java.io.Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4725055069068032253L;
	/**
	 * 一层设备的用户
	 */
	@JsonIgnore
	...
	//输入状态的值，-1表示没有端口，默认是没有端口的
	private int[] inputState=new int[]{-1,-1,-1,-1,-1,-1,-1,-1};
	/*** 输出状态 ***/
	private  String  field15;
	//输出状态的值，-1表示没有端口，默认是没有端口的
	private int[] outState=new int[]{-1,-1,-1,-1,-1,-1,-1,-1};
...
/////   input  end !
	...
	/**屏蔽状态（1正常，3掉线，2为屏蔽禁用）Constant的 状态**/
	...
	private Integer dtype;		//设备类型(设备类型决定 了设备 的输入 与输出 个数）
	@JsonIgnore
	private Table10 parent;
	
	private int inCount=0; //输入端口个数
	private int outCount=0;//输出端口个数
	 ...
	 * @param field121
	 */
	public Table10(Object field3,Object field121) throws Exception{
		this.field3=NumberUtils.createInteger(field3.toString());
		this.field121=field121+"";
	}
	/** full constructor */
	public Table10( Integer  field2,  Integer  field3,  Integer  field4,  Integer  field5,
	...
	public String getField121() {
		if(field121==null||field121.trim().equals("")){
			if(field4==0)
			{
				field121="一层设备"+field3;
			}else{
				field121="二层设备"+field3+"-"+field4;
			}
	...
	@JsonIgnore
	public List<Table10> getSecond() {
		if(second==null) this.second=new ArrayList<Table10>();
		return second;
	}
	public void setSecond(List<Table10> second) {
		this.second = second;
	}
	/**
	 * 准备128台二层设备
...
/*	public void loadSecond(){
	...
	/*	Field[] fields= this.getClass().getFields();
		for (int j = 0; j < fields.length; j++) {
			if(!fields[j].getName().equalsIgnoreCase("field1")){
				try {
					
					fields[j].set(this,fields[j].get(info));
		...
		for (int i = 0; i < ms.length; i++) {
			if(ms[i].getName().indexOf("set")<1&&!ms[i].getName().equals("setField1")){
				try 
				{
					...
					if(obj!=null)
					MethodUtils.invokeExactMethod(this, ms[i].getName(),new Object[]{obj});
	 ...
	 * @param t10
	 */
	public void copyAllFrom(Table10 t10){
		Field[] fields= this.getClass().getDeclaredFields();
		for (int j = 0; j < fields.length; j++) {
				try {
					if(Modifier.isStatic(fields[j].getModifiers()))continue;
					
					if( fields[j].get(t10)   instanceof java.util.List ){
					}else{
						//fields[j].setAccessible(true);
						//t10.getClass().getDeclaredField(fields[j].getName()).setAccessible(true);
						fields[j].set(this,fields[j].get(t10));
					}
				} catch (IllegalArgumentException e) {
					e.printStackTrace();
				} catch (IllegalAccessException e) {
	 ...
	 * @return
	 */
	public Integer loadPortInputValue(int input) {
		switch (input) {
		case 0:
			return field17!=null?field17:-1;

		case 1:
			return field18!=null?field18:-1;

		case 2:

			return field19!=null?field19:-1;

		case 3:

			return field20!=null?field20:-1;

		case 4:

			return field21!=null?field21:-1;

		case 5:
			return field22!=null?field22:-1;

		case 6:

			return field23!=null?field23:-1;

		case 7:

			return field24!=null?field24:-1;
	 ...
	 */
	@JsonIgnore
	public String findPortInputName(int input) {
		switch (input) {
		case 0:
			return StringUtils.isNotBlank(field105)?field105:buldInPort(input+1);

		case 1:
			return StringUtils.isNotBlank(field106)?field106:buldInPort(input+1);

		case 2:

			return StringUtils.isNotBlank(field107)?field107:buldInPort(input+1);

		case 3:

			return StringUtils.isNotBlank(field108) ?field108:buldInPort(input+1);

		case 4:

			return StringUtils.isNotBlank(field109) ?field109:buldInPort(input+1);

		case 5:
			return StringUtils.isNotBlank(field110 )?field110:buldInPort(input+1);

		case 6:

			return !StringUtils.isNotBlank(field111) ?field111:buldInPort(input+1);

		case 7:

			return !StringUtils.isNotBlank(field112) ?field112:buldInPort(input+1);
	 ...
	 * @return
	 */
	public String loadPortOutputName(int output) {
		switch (output) {
		case 0:
			return StringUtils.isNotBlank(field113) ?field113:buldOutPort(output+1);

		case 1:
			return StringUtils.isNotBlank(field114) ?field114:buldOutPort(output+1);

		case 2:

			return StringUtils.isNotBlank(field115) ?field115:buldOutPort(output+1);

		case 3:

			return StringUtils.isNotBlank(field116) ?field116:buldOutPort(output+1);

		case 4:

			return StringUtils.isNotBlank(field117) ?field117:buldOutPort(output+1);

		case 5:
			return StringUtils.isNotBlank(field118)?field118:buldOutPort(output+1);

		case 6:

			return StringUtils.isBlank(field119)?field119:buldOutPort(output+1);

		case 7:

			return StringUtils.isBlank(field120)?field120:buldOutPort(output+1);
	...
	private String buldOutPort(int portNum){
		if(field3==0){
			return"一层设备"+StringUtils.leftPad(""+field3,3,'0')+portNum+"输出";
		}else{
			return"二层设备"+StringUtils.leftPad(""+field4,3,'0')+portNum+"输出";
		}
		
	}
	private String buldInPort(int portNum){
		if(field3==0){
			return"一层设备"+StringUtils.leftPad(""+field3,3,'0')+portNum+"输入";
		}else{
			return"二层设备"+StringUtils.leftPad(""+field4,3,'0')+portNum+"输入";
		}
	 ...
	 * @return
	 */
	public int getPortState(int type,int portIndex){
		if(portIndex<1||portIndex>8)return -1;
		if(type==1){
			return inputState[portIndex-1];
		}else{
			return outState[portIndex-1];
		}
		}
	/**
	 * 
	 ...
	 * @param value
	 */
	public void setPortState(int type,int value){
	 ...
	 * @lastUpdate 2014-4-7
	 */
	public void distoryPort(boolean timeOut) {
		if(timeOut)
		 for (int i = 0; i < 8; i++) {
			outState[i] = -1;
			inputState[i] = -1;
	 ...
	 * @param state
	 */
	public void setPortState(int type,int portIndex,int state){
		if(portIndex<1||portIndex>8)return ;
		if(type==1){
			  inputState[portIndex-1]=state;
		}else{
			  outState[portIndex-1]=state;
		}
		}
	// Property accessors
	/**
